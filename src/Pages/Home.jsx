...
const Home = () => {
  ...
  useEffect(()=>{
   const fetchAllTodos = async()=>{
          const res = await axios.get('https://server-lc3v.onrender.com/api/v1/todos');
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      const res = await axios.post('https://server-lc3v.onrender.com/api/v1/todos',{title,isCompleted});
      setRender(!render);
  ...
  const handleDeleteTodo = async(id) => {
    try {
      const res = await axios.delete(`https://server-lc3v.onrender.com/api/v1/todos/${id}`);
      setRender(!render);
      alert(res.data.message);
    } catch (error) {
      console.log(error.message);
  ...
  const handleEditTodo = async(id) => {
    ...
    const res = await axios.get(`https://server-lc3v.onrender.com/api/v1/todos/${id}`)
    let todoToEdit= res.data.data;
    setTitle(todoToEdit.title);
    setIsCompleted(todoToEdit.isCompleted);
     await axios.delete(`https://server-lc3v.onrender.com/api/v1/todos/${id}`);
      setRender(!render);
    try {
   
      const res = await axios.put(`https://server-lc3v.onrender.com/api/v1/todos/${id}`,{ title:todoToEdit.title,
